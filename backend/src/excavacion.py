...
def excavar(repo_url: str, token: str) -> dict[str, Any]:
    """Devuelve {repo, narrativa, html, commits}. Lanza ArqueologoError."""
    ...
    relato, uso = narrator.narrar(meta, readme, commits)
    html = renderer.render(relato, repo, commits)

    log.info(
        "excavado repo=%s commits=%d readme_chars=%d tokens_in=%s tokens_out=%s duracion=%.1fs",
        repo,
        len(commits),
        len(readme),
        uso.get("inputTokens"),
    ...
    return {"repo": repo, "narrativa": relato, "html": html, "commits": commits}
